...
// 3. Konstans ellenőrzés (wp-config.php)
add_action( 'admin_notices', function() {
    $required = [ 'O365_TENANT_ID', 'O365_CLIENT_ID', 'O365_CLIENT_SECRET', 'O365_SENDER_EMAIL' ];
    $missing = [];
    foreach ( $required as $const ) {
        if ( ! defined( $const ) || empty( constant( $const ) ) ) $missing[] = $const;
    }
    if ( ! empty( $missing ) ) {
        printf( '<div class="notice notice-error"><p>O365 Calendar Error: Hiányzó konstansok: <strong>%s</strong></p></div>', implode( ', ', $missing ) );
    }
});

// 4. Gyorsítótár ürítése gomb a plugin listában
...
// 8. REST API & Frissítés ellenőrző
